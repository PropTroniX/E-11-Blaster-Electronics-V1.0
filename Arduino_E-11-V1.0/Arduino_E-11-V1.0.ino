...
const int magPin = 6;                // Magazine Detection Switch
...
int mag_loaded = LOW;               // set magazine pin to low
...
void setup() {
  ...
  pinMode(magPin, INPUT);            // set magazine button as input
...
void loop() {
// Start of Magazine Detection
  magInState = digitalRead(magPin);   // Magazine
  if (magInState == LOW)
  {
    mag_out ();
  } else {
...
void sub_code () {      // Ammo Magazine Detection
  if (mag_loaded) {
    delay (200);
    mp3_play (29);
    delay (50);
